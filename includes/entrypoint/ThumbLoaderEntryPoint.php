...
class ThumbLoaderEntryPoint {

// Keep indentation for git history continuity.
public static function main() {
	...
	if ( MW_ENTRY_POINT === 'thumb_handler' ) {
		...
		self::StreamThumb( $wgRequest->getQueryValuesOnly() );
	}
 ...
 * @return void
 */
private static function ThumbHandle404() {
	global $wgThumbPath;
	...
	$params = self::ExtractThumbRequestInfo( $relPath ); // basic wiki URL param extracting
	if ( $params == null ) {
		self::ThumbError( 400, 'The specified thumbnail parameters are not recognized.' );
		return;
	}

	self::StreamThumb( $params ); // stream the thumbnail
}
 ...
 * @return void
 */
private static function StreamThumb( array $params ) {
	global $wgVaryOnXFP;
	...
	if ( $isTemp ) {
		...
		if ( count( $bits ) != 2 ) {
			self::ThumbError( 404, wfMessage( 'badtitletext' )->parse() );
			return;
		}
	...
	if ( !$img ) {
		self::ThumbError( 404, wfMessage( 'badtitletext' )->parse() . "<br>\n" . '<b>FileName:</b> ' . htmlspecialchars( $fileName ) );
		return;
	}
	...
	if ( !in_array( 'read', $permissionManager->getGroupPermissions( [ '*' ] ), true ) ) {
		...
		if ( !$imgTitle || !$permissionManager->userCan( 'read', $user, $imgTitle ) ) {
			self::ThumbError( 403, 'Access denied. You do not have permission to access ' .
				'the source file.' );
			return;
		}
	...
	if ( $img->isDeleted( File::DELETED_FILE ) ) {
		self::ThumbErrorText( 404, "The source file '$fileName' does not exist." );
		return;
	}

	// Do rendering parameters extraction from thumbnail name.
	if ( isset( $params['thumbName'] ) ) {
		$params = self::ExtractThumbParams( $img, $params );
	}
	if ( $params == null ) {
		self::ThumbError( 400, 'The specified thumbnail parameters are not recognized.' );
		return;
	...
	if ( !$img->exists() ) {
		...
		self::ThumbErrorText( 400, "The source file '$fileName' is not locally accessible." );
		return;
	}
	...
	try {
		...
		self::ThumbError( 500, $e->getHTML(), 'Exception caught while extracting thumb name',
			[ 'exception' => $e ] );
		return;
	}
	...
	if ( $rel404 !== null ) { // thumbnail was handled via 404
		if ( rawurldecode( $rel404 ) === $img->getThumbRel( $thumbName ) ) {
			...
			self::ThumbErrorText( 404, "The given path of the specified thumbnail is incorrect;
	...
	if ( $img->getRepo()->fileExists( $thumbPath ) ) {
		...
		if ( $status->isOK() ) {
			...
			self::ThumbError( 500, 'Could not stream the file', null, [ 'file' => $thumbName,
	...
	if ( !wfThumbIsStandard( $img, $params ) && $user->pingLimiter( 'renderfile-nonstandard' ) ) {
		self::ThumbError( 429, wfMessage( 'actionthrottledtext' )->parse() );
		return;
	...
	if ( strlen( $thumbProxyUrl ) ) {
		self::ProxyThumbnailRequest( $img, $thumbName );
		// No local fallback when in proxy mode
		return;
	} else {
		// Generate the thumbnail locally
		list( $thumb, $errorMsg ) = self::GenerateThumbnail( $img, $params, $thumbName, $thumbPath );
	}
	...
	if ( $errorMsg !== false ) {
		self::ThumbError( $errorCode, $errorMsg, null, [ 'file' => $thumbName, 'path' => $thumbPath ] );
	} else {
		// Stream the file if there were no errors
		$status = $thumb->streamFileWithStatus( $headers );
		if ( !$status->isOK() ) {
			self::ThumbError( 500, 'Could not stream the file', null, [
 ...
 */
private static function ProxyThumbnailRequest( $img, $thumbName ) {
	$thumbProxyUrl = $img->getRepo()->getThumbProxyUrl();
 ...
 */
private static function GenerateThumbnail( File $file, array $params, $thumbName, $thumbPath ) {
 ...
 *
 * Transform specific parameters are set later via self::ExtractThumbParams().
 *
 * @param string $thumbRel Thumbnail path relative to the thumb zone
 * @return array|null Associative params array or null
 */
private static function ExtractThumbRequestInfo( $thumbRel ) {
	$repo = MediaWikiServices::getInstance()->getRepoGroup()->getLocalRepo();
 ...
 */
private static function ExtractThumbParams( $file, $params ) {
	if ( !isset( $params['thumbName'] ) ) {
 ...
 * @return void
 */
private static function ThumbErrorText( $status, $msgText ) {
	self::ThumbError( $status, htmlspecialchars( $msgText, ENT_NOQUOTES ) );
}
 ...
 * @return void
 */
private static function ThumbError( $status, $msgHtml, $msgText = null, $context = [] ) {
